...
export const methodsApi = {
  ...
  get: async (id: number, owner?: string): Promise<Method | null> => {
    if (owner) {
      ...
      if (ownerMethod) return { ...ownerMethod, is_public: false };
      return null;
    }
  ...
  // so a receiver editing a shared method can still fork it. Flagged for the
  // user: this could alternatively fork into the owner's dir; defaulting to
  // self-fork to match the receiver-creates-their-own-copy intent.
  fork: async (
    id: number,
    data: { new_name: string; new_github_path: string; deviations: string },
    owner?: string
  ): Promise<Method> => {
