...
export async function parseImportBundle(file: Blob): Promise<ImportPayload> {
  ...
  for (const [path, entry] of Object.entries(zip.files)) {
    ...
    const codingWorkflowMatch = path.match(METHOD_CODING_WORKFLOW_RE);
    if (codingWorkflowMatch) {
      const id = Number(codingWorkflowMatch[1]);
      try {
        const protocol = JSON.parse(await entry.async("string")) as CodingWorkflowProtocol;
        methodCodingWorkflows.set(id, protocol);
      } catch (err) {
        console.warn(`[import.parse] failed to parse ${path}:`, err);
      }
      continue;
