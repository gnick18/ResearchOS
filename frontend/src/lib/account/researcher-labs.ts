 ...
 */
/**
 * The shared lab-collection join, keyed by a resolved owner_key.
 *
 * Two paths in one LEFT JOIN query:
 *   Path A (member): billing_lab_members where member_owner_key = ownerKey AND
 *     status = 'active', joined to directory_labs (name/institution) + lab_sites
 *     (slug).
 *   Path B (PI): directory_labs where pi_email_hash = ownerKey (the PI is not in
 *     billing_lab_members for their own lab).
 *
 * `listedOnly` applies the PUBLIC privacy filter (directory_labs.listed = true).
 * Pass false ONLY for the owner's own authenticated view, where unlisted own labs
 * are visible (the listed flag governs what OTHERS discover on your public
 * profile, not what you see of your own labs).
 */
async function labsForOwnerKey(
  sql: NeonQueryFunction<false, false>,
  ownerKey: string,
  listedOnly: boolean,
): Promise<ResearcherLabEntry[]> {
  // showAll short-circuits the listed filter for the owner's own rail.
  const showAll = !listedOnly;
  const rows = (await sql`
    ...
    WHERE (${showAll} OR dl.listed = true)
      AND (
...
export async function getResearcherPublicLabs(
  ...
  // Step 2: the listed-only join (public profile privacy rule).
  return labsForOwnerKey(sql, ownerRows[0].owner_key, true);
}

/**
 * Returns ALL labs the given owner key belongs to (member or PI), INCLUDING
 * unlisted ones, for the owner's OWN authenticated surface (the /network left
 * rail). The public listed-only filter does not apply here, the listed flag is
 * about what other people discover on your public profile, not what you see of
 * your own labs (Grant decision 2026-06-20). Takes the owner key directly since
 * the caller already holds it from the session, so no handle round-trip.
 */
export async function getResearcherOwnLabs(
  ownerKey: string,
): Promise<ResearcherLabEntry[]> {
  if (!ownerKey) return [];
  const sql = getSql();
  return labsForOwnerKey(sql, ownerKey, false);
}
