...
// "Rotating presenter / journal-club schedule" paragraph).
//
// A thin per-user store for `CheckinRotation` records, MIRRORING the compact
// store (lib/checkins/compact-store.ts): JsonStore's on-disk layout
// (`users/<owner>/checkin_rotations/<id>.json`) keyed on a globally-unique
// crypto.randomUUID string so the rotation id never collides across owners.
//
// HOME FOLDER: a rotation hangs off a GROUP check-in space and lives in the
// SPACE OWNER's folder (`owner === space.owner`). Every member is in
// `shared_with` at "edit" so any member can advance or reorder a track. There is
// at most one rotation per space (looked up by `space_id` via `listAllForUser`).

import { fileService } from "../file-system/file-service";
import { getCurrentUserCached } from "../storage/json-store";
import type { CheckinRotation } from "../types";
...
export class CheckinRotationStore extends StringKeyedStore<CheckinRotation> {
  constructor() {
    super("checkin_rotations");
  }
}
