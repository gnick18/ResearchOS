...
export function plotLayoutManifest(
  ...
  if ("clusters" in geometry && "legend" in geometry) {
    ...
    // The legend block (top-right INSIDE the plot area), measured with the SAME
    // constants the serializer draws from. Left edge = swatch x; right edge = the
    // series-name start + the widest name; height = one row per entry.
    if (geo.legend.length > 0) {
      ...
      boxes.push({
        ...
        w:
          ...
          GROUPED_LEGEND.textInsetFromX1 +
          maxNameW,
