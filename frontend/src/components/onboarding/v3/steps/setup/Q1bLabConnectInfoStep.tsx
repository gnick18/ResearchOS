 ...
 * picker mid-wizard requires tearing down file-system context + re-
 * initialization, which would either kill the wizard's resume state
 * or interrupt the user's flow in unpredictable ways. Honest path:
 * tell the user to install the provider + close-reopen ResearchOS +
 * pick the lab folder when prompted. If they want to keep going now,
 * the temp-local option keeps the wizard alive on the current folder
 * with migration deferred.
 */
