 ...
 * app. The demo clicks the first rendered widget tile, the popup opens,
 * BeakerBot lets the user read for a beat, and the cursor then closes
 * it again.
 *
 * Cursor + spotlight:
 *   - spotlight on the first widget tile (prefix-matched via
 *     `home-widget-tile-` so the demo picks whichever default landed
 *     first; Chip A pre-seeded `sidebar-upcoming` + `calendar-events-
 *     today`, so the first tile is the Upcoming tasks widget).
 *   - cursor clicks the tile, the SnapshotTilePopup mounts, the user
 *     reads, and then a deferred click on the popup's Close (X) button
 *     dismisses the popup so the canvas is visible again for the next
 *     step.
 *
 ...
 *   resolve null and the close action would never fire. `deferredClickAction`
 *   re-queries the selector at PLAYBACK time so it runs after the
 *   popup mount.
 *
...
/**
 * Prefix selector for the first rendered widget tile. Chip A's pre-seed
 * lands `sidebar-upcoming` first; under that default, the demo opens
 * the Upcoming tasks popup. If the pre-seed list changes in the future,
 * the prefix match still resolves to whatever tile actually rendered
 * first — the step body doesn't pin a specific widget id, only "the
 * first tile on the canvas".
 *
 * Exported so the registry test can assert the selector shape if it
 * ever wants to.
 */
 ...
 * shared across surfaces — adding a stamp here would also fire on the
...
export const homeWidgetsTileAnatomyStep = buildWalkthroughStep({
  ...
  speech:
    "Each tile shows you a snapshot. The numbers and the top few rows are enough at a glance. Click the tile to expand it into a full popup, where you get filters, search, and the same actions you'd find on the dedicated page.",
  pose: "pointing",
  targetSelector: HOME_WIDGETS_TILE_ANATOMY_TILE_SELECTOR,
  cursorScript: cursorScript(async () => {
    // Click the first rendered tile. The popup mounts synchronously
    // on the next React commit, so the deferred-close action can
    // resolve the popup's close button on its first re-query.
    ...
    // dismisses it. 1800ms is a comfortable read of the title +
    // first-row summary without dragging the step.
