/**
 * §6.16 Phase 2c Lab Mode tour — lab-mode-gantt step tests.
 *
 * Lab Mode fix manager R1 (2026-05-22): cursor demo chains tab
 * click → deferred bar click → deferred popup close.
 */
...
describe("labModeGanttStep cursor demo", () => {
  beforeEach(() => {
    document.body.innerHTML = "";
  });

  it("returns [] when the tab anchor is missing", async () => {
    const actions = await labModeGanttStep.cursorScript?.();
    expect(actions).toEqual([]);
  });

  it("produces three actions (tab click + deferred bar click + deferred popup close)", async () => {
    document.body.innerHTML = `
      <button data-tour-target="lab-mode-gantt-tab">gantt tab</button>
    `;
    const actions = await labModeGanttStep.cursorScript?.();
