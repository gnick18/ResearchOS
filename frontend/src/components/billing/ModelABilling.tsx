...
export interface ModelAStatus {
  ...
  trialPaused?: boolean;
  /**
   * The lab that grants this member premium, or null. Present only when a lab
   * actively sponsors the signed-in user (its PI is on a paid or comped plan).
   * When set, the member HAS premium because their lab head pays for it, so the
   * panel leads with that rather than the member's own free or solo plan. tier is
...
function Ready({
  ...
  // A member of a paid or comped lab HAS premium because their lab head pays for
  // it, so that is the headline, not the member's own free or solo plan. We do
  // not show them personal billing controls (card, cap, accrual), since the lab
  // is billed, not them. They can still make their own separate folders under
  // this access.
  if (status.sponsoringLab) {
    return <LabPremiumPanel labName={status.sponsoringLab.name} />;
  }
...
// ---- Member premium via their lab ----
//
// When a lab head pays for a member, the member HAS premium. The badge says so
// plainly (Grant's rule: being in a paid lab makes you premium, full stop), and
// we name the lab so it is clear who covers them and that the lab head, not the
// member, is billed. No personal card or cap, the lab pool handles billing.
function LabPremiumPanel({ labName }: { labName: string }) {
  return (
    <div className={card}>
      <div className="flex items-start gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-md bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-800 dark:bg-green-500/15 dark:text-green-300">
              via {labName}
            </span>
          </div>
          <p className="mt-2 max-w-prose text-sm leading-relaxed text-foreground-muted">
            You have full premium access because {labName} covers you. Sending
            outside your folder, live co-editing, and app pairing are all
            included. Your lab head is billed for the shared lab pool, never you.
          </p>
          <p className="mt-2 max-w-prose text-sm leading-relaxed text-foreground-muted">
            You can still make your own separate folders, your premium access
            comes with you.
          </p>
        </div>
      </div>
