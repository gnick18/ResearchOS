...
// Cross-boundary sharing, the standalone SEQUENCE "Share outside this folder"
// send dialog (sequences tier, the simplest tier).
//
// Sends ONE sequence to ONE recipient as an encrypted snapshot. A SEPARATE
// component mirroring MethodSendOutsideDialog.tsx, the note / experiment / method
// dialogs must not be edited. A sequence is self-contained, just GenBank text
// plus a tiny meta, with no attachments and no lineage, so this is the smallest
// of the dialogs, no compound / cannot-share state, just the identity gate plus
// the send / invite paths. The only real differences from the method dialog are
// the summary line ("Sending this sequence") and the payload builder
// (buildSequenceSendPayload instead of buildMethodSendPayload).
//
...
// invite-a-non-user path the method / note dialogs use.
...
interface SequenceSendOutsideDialogProps {
  /** The sequence to send. Its current snapshot is what the recipient gets. */
...
function SendForm({
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      // Build the small JSON envelope carrying this sequence's GenBank text +
  ...
  }, [sequence, ownerUsername, recipient, senderEmail]);

  // Invite the non-user, seal the sequence envelope under a one-time key, park it
  // on the relay, and have ResearchOS send the branded email. The title is the
  // only content exposed in the email. Mirrors the method dialog's invite.
  const handleInvite = useCallback(async () => {
    ...
    try {
      ...
      const result = await inviteRawShare({
        ...
        recipientEmail,
        payload,
        itemTitle: sequence.display_name || "Untitled sequence",
  ...
  // them and share this sequence. The copy states the lower-assurance trust
      ...
      <div className="space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-body text-foreground-muted mt-1 leading-relaxed">
              ...
              free account. The email holds no key, so it cannot open the sequence
              ...
              sequence stays encrypted until they open it with that key.
            </p>
          </div>
        </div>
        <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 rounded-lg">
          ...
          <p className="text-meta text-amber-800 dark:text-amber-300 leading-relaxed">
            ...
            trust. Whoever holds that key can open the sequence, so send it
  ...
  return (
    <div className="space-y-4">
      <div className="bg-surface-sunken border border-border rounded-lg px-3 py-2.5">
        <p className="text-meta text-foreground-muted">Sending this sequence</p>
        <p className="text-body font-medium text-foreground mt-0.5 break-words">
          {sequence.display_name || "Untitled sequence"}
        </p>
      </div>

      <p className="text-body text-foreground-muted leading-relaxed">
        This sends an encrypted copy, a snapshot of the sequence as it looks now,
        its GenBank file with every feature and annotation. It is not live shared
        editing, the recipient gets their own copy.
      </p>

      <div>
        <label
          htmlFor="sequence-send-outside-recipient"
          className="block text-meta font-medium text-foreground mb-1"
        >
          Recipient email
        </label>
        <input
          id="sequence-send-outside-recipient"
          type="email"
          value={recipient}
          onChange={(e) => {
