...
export default function StickyEndLadderHero({ product, pieces, enzymeNames }: Props) {
  ...
  return (
    ...
    >
          ...
          {junctions.map((jn, i) => {
            ...
            return (
              <div key={i} className="rounded-md border border-gray-200 bg-white p-2">
                ...
                {jn.kind !== "blunt" ? (
                  <div className="mt-1 text-center font-mono text-[11px] font-medium text-sky-700">
                    {jn.overhang}
                  </div>
                ) : null}
              </div>
      ...
      {flags.length > 0 ? (
        <div className="mt-2.5 flex items-start gap-1.5 rounded bg-amber-50 px-2 py-1.5 text-[11px] text-amber-700">
