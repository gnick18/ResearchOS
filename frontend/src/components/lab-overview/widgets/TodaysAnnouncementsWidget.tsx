...
// ─────────────────────────────────────────────────────────────────────────────
// Inline pin SVG preserved on the body; SidebarTile uses the same
// shape so the visual cue is consistent across surfaces.
...
export function SidebarTile({ onClick }: SidebarTileProps) {
  ...
  return (
    <SidebarStatTile
      ...
      label="Pinned"
      stat={isLoading ? "—" : pinned}
      sub={pinned === 0 ? "Nothing pinned" : undefined}
      onClick={onClick}
    />
  );
}
