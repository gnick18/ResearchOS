...
### New open questions surfaced during the unified-sharing audit

- **`is_public` on Method and Mass spec protocol; keep or kill?** The migration writes `is_public === true` records as `shared_with: [{ username: "*", level: "read" }]`. After that, the `is_public` boolean is redundant; derived from `shared_with`. **Question:** drop `is_public` from disk in the same migration, or leave it as a derived view-only field for one release? Recommendation: keep on disk for one release as a safety net (so older clients still read sharing correctly), then drop in R7-cleanup.
- **`hide_goals_from_lab` opt-out; semantic equivalent in unified shape?** Currently a per-user-settings boolean that hides ALL of the user's goals from `LabRoadmapsPanel`. The unified migration writes `shared_with: []` (owner-only) per-goal IFF the user had `hide_goals_from_lab === true` at migration time. Going forward, opt-out is per-goal: the user can share goal A with the lab and keep goal B private. **Question:** is per-goal granularity the right replacement, or does Grant want a per-user default like the old setting? Recommendation: per-goal; strictly more powerful; the user can replicate the old behavior by sharing nothing.
- **Project sharing semantics; does sharing a project share its tasks?** Currently `Project.shared_with` is a hint, but each Task carries its own `shared_with`. **Question:** when sharing a project with `level: "read"`, do we cascade `level: "read"` onto every Task in that project? Recommendation: NO cascade; too surprising. The "share the project" surface in `<ShareDialog>` should warn ("Tasks in this project are NOT auto-shared; share them individually") and offer a checkbox "also share all tasks in this project" that does the explicit fan-out on confirm.
- **Methods that are referenced by a shared task but not shared themselves.** If @morgan shares a PCR task with me and that task references a private method, can I see the method? Today: no (method's own `shared_with` gates it). **Question:** auto-grant transient `level: "read"` on referenced methods when the parent task is shared? Recommendation: YES with an audit-log entry; the alternative ("you can see the task but the method link is broken") is a worse user experience. Cap the depth at 1 (don't recurse into compound method components).

These are surfaced for Grant's resolution before Phase R1 implementation kicks off.
