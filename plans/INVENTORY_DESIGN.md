...
On an unknown `product_barcode`, attempt an online lookup to pre-fill name and vendor. CORS must be checked per source (see `[[reference_zenodo_figshare_cors]]`); a browser-direct call is preferred, otherwise a thin `/api/barcode-lookup` proxy route on the existing proxy pattern with the rate-limit wrapper.

RESEARCH UPDATE 2026-06-07 (`docs/research/barcode-lookup-apis.md`). The lookup is LOW VALUE for lab reagents and should be demoted. Lab reagents from NEB / Sigma-Aldrich / Thermo Fisher overwhelmingly do NOT carry standard retail UPC/EAN/GTIN codes in consumer databases; they are identified by proprietary vendor catalog numbers (and CAS for chemicals). Real chemical-inventory tools (ChemInventory) bind a LAB-ASSIGNED internal code to each container rather than reading a manufacturer code. So the high-value scan path is the per-container `container_code` (a lab-applied label), with the manufacturer-barcode lookup as a rare bonus. DECISION (Grant 2026-06-07, refines B3): make manual entry plus bind-on-scan via the lab-applied `container_code` the PRIMARY scan path, and ship the manufacturer-UPC online lookup behind a flag, default off, with NO bundled API key (advanced users paste their own Go-UPC key in settings; a shared baked-in key would leak and burn the quota). Go-UPC is the only candidate with browser-direct evidence (free tier ~150/mo); UPCitemdb's keyless trial (100/day) is the zero-config alternative if its CORS checks out. Dismiss GS1 (paywalled enterprise, not browser-safe) and the food-only DBs (Open Food Facts, Nutritionix). CORS is unverified for every candidate and needs a live check before wiring browser-direct vs proxy.
