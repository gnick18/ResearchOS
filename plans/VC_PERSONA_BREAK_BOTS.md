...
- Preview MCP preamble: fix the worktree .claude/launch.json to a direct launcher on your port (bash -c "cd <worktree>/frontend && exec npm run dev -- -p 341N"); preview_start; preview_resize to 1440x900; sanity-check the live bundle has the version-history button (preview_eval). Navigate to http://localhost:341N/demo (the INTERACTIVE demo: auto-loads the fixture as alex a PI, no folder picker, and notes are EDITABLE + writes persist in-memory for the session). Do NOT use ?wikiCapture=1 - that is the read-only SCREENSHOT fixture (no editor mounts, notes uneditable), which cannot generate history. NEVER touch real data.
...
### 5. Restart / re-run user (stress + PI)
Restores and undoes many times in a row; makes many rapid saves (push toward compaction if reachable); checks the timeline stays coherent + the undo window behaves after repeated restores. THEN the PI path (alex is a PI in fixture): try to view + restore a DIFFERENT user's note (morgan/mira) - confirm Restore is hidden or disabled-with-unlock-tooltip when not unlocked, and (if unlock is reachable in fixture) that a PI restore routes correctly. Surfaces: repeated-restore integrity, undo-window-after-many-restores, the PI gate (hidden vs disabled vs enabled), cross-owner restore behavior, compaction-boundary weirdness.
