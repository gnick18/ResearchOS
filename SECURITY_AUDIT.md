...
3. **`research-os-telegram-token-cache` / object-store `tokens`** (new; recovery-cache for a misshared-disk scenario where `users/<u>/_telegram.json` is unreadable or absent but the browser still holds the last-known credentials for this user-in-this-folder). Compound key `{folderName, username}`, minimal payload `{bot_token, chat_id, bot_username}` (no `last_update_id`, no message bodies, no inbox photos).
   ...
   - **Offline-mode interaction.** Offline mode does not block reads from this DB (the entire point is a local-only recovery surface). It does block the `getMe` validation step. Under offline mode the recovery prompt still renders so the user sees the affordance exists, but the Reconnect action is disabled with a Tooltip explaining the user should disable offline mode in Settings to proceed.
...
**Update (post-fix, 2026-05-19): `'wasm-unsafe-eval'` added to `script-src`.** The CSP that landed at `03f77091` shipped without `'wasm-unsafe-eval'`, which broke PDF export with `CompileError: WebAssembly.instantiate(): Compiling or instantiating WebAssembly module violates the following Content Security policy directive`. `@react-pdf/renderer` reaches `@react-pdf/layout`, which depends on `yoga-layout`; the browser build of yoga-layout ships as a base64-inlined WASM module compiled at runtime. (The original CSP work assumed the WASM source would be `fontkit-wasm` for runtime font subsetting; the actual blocker is yoga-layout for flex layout — fontkit in our tree is pure-JS.) The fix is `'wasm-unsafe-eval'` on `script-src`, not `'unsafe-eval'`. The difference matters: `'wasm-unsafe-eval'` allows `WebAssembly.compile` and `WebAssembly.instantiate` only; `'unsafe-eval'` would additionally re-open `eval()`, `new Function()`, and `setTimeout(string, ...)`, all of which are useful primitives for an XSS-via-stored-content attacker. The narrow opt-in keeps the textual-eval doors closed.

**WASM threat surface — honest read.** WebAssembly's threat surface is meaningfully smaller than JS: a WASM module has no direct DOM access, runs in a sandboxed linear memory, and can only call back into the host through explicitly bound imports. An attacker who could already inject WASM bytes via a stored-XSS vector would gain the ability to compile and run them with `'wasm-unsafe-eval'` set, but they would still need a separate path to reach the DOM or fetch — the §3.1 sanitize work plus the existing `connect-src 'self' https://api.telegram.org https://vitals.vercel-insights.com` allowlist keep those vectors closed. No user-content path (markdown, ELN import, Telegram inbox content, file-system reads) currently accepts opaque binary blobs that the page would reinterpret as WASM bytes, so the practical exposure of this directive in our threat model is the @react-pdf/renderer WASM byte stream itself, which is statically bundled at build time.

**Bundle WASM-source audit (2026-05-19).** A repo-wide `grep -rl "WebAssembly\\.compile\\|WebAssembly\\.instantiate" frontend/node_modules/ --include="*.js"` plus `find frontend/node_modules/ -name "*.wasm"` against an installed worktree returned: yoga-layout's `yoga-wasm-base64-esm.js` is the only runtime WASM consumer that reaches the user-facing browser bundle (via `@react-pdf/layout` → `yoga-layout`). Other hits are all build-time or server-side surfaces that never ship to the client: `next/dist/compiled/source-map08` (dev error-overlay sourcemap parsing), `next/dist/esm/server/web/*` and `next/dist/compiled/edge-runtime/*` and `next/dist/compiled/@edge-runtime/primitives/*` (edge-runtime sandbox, server-only), `next/dist/compiled/@vercel/og/*` (only loaded inside an `/api/og` route — ResearchOS does not use `@vercel/og`), `next/dist/compiled/webpack/bundle5.js` and `styled-jsx/dist/babel/index.js` (build-time webpack/babel), `@emnapi/core` and `@tybys/wasm-util` (build-time node-addon helpers), `vitest` / `vite` / `@vitest/mocker` / `es-module-lexer` (test/build tools). Loose `.wasm` files in the tree are `next/dist/compiled/source-map08/mappings.wasm` (dev error overlay) and `next/dist/compiled/@vercel/og/{yoga,resvg}.wasm` (unused). yoga-layout uses base64-inlined bytes rather than a `.wasm` sibling, which is why `find -name "*.wasm"` does not surface it directly. Net: `'wasm-unsafe-eval'` is enabling exactly one path (yoga-layout for PDF layout) today; if a future dependency adds a WASM-using package — image decoder, parquet reader, crypto primitive, font subsetter — security manager should re-audit and decide whether it belongs in the user-facing bundle.
