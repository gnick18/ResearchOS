...
- **C1 — reset-keep-data escape ✅ DONE.** Primitive `resetIdentityKeepData(username)` (`storage.ts`) + unit tests, **plus the unlock-gate UI** (`UserLoginScreen.tsx`, merge `708a901db`): a "Can't sign in? Reset and keep your data" affordance on the unlock gate opens a two-step warning confirmation (kept: all notebook data; lost: old signing identity + access to data shared TO the old key; shared lab needs PI re-admit), then runs the primitive and surfaces the new recovery code through the existing createdRecovery display (signs in on continue). Client-side only, no new backend. Dark behind `NEXT_PUBLIC_MULTI_FOLDER` (the confirm state's only setter is the flag-gated button → flag-OFF byte-identical). tsc 0; identity suite 100 tests. **BROWSER-VERIFIED 2026-06-15 (collaborative Chrome verify, all steps PASS):** reset link present verbatim, Kept/Lost warning correct, new recovery code minted + differs from the original, the "SURVIVES RESET" note survived the reset intact, signed in with the fresh identity (folder stayed connected). Only the OS folder-picker step was human-done.
- **C2 — PI re-admit (PRIMITIVE DONE).** After a reset, the member's NEW public key is re-admitted to the lab roster by the lab head. Primitive `readmitMember(record, currentLabKey, username, newKeys, headPriv)` (`lib/lab/lab-key.ts`, merge `f0b30f722`) — composed from the existing audited `rotateLabKey` (evict stale keys, new generation, seed-linked) + `addMember` (re-add same username with new keys), so it introduces NO new signed-log event type (the tamper-evident `canonicalEntryMessage`/`verifyMembershipLog` schema is untouched). Lost old key is excluded from post-readmit data; member regains current+historical lab data via the new key + seed chain; role + email binding preserved. 3 unit tests; lab-key suite 20 pass; tsc 0. **Increment 2 — PI-facing UI DONE** (merge `07eb0f811`). `readmitMemberRemote` (`lab-do-client.ts`) — the two-append remote counterpart (`rotateLabKey`→`appendRotateRemote`→`addMember`→`appendAddMemberRemote`); non-atomic, so partial-failure is explicit (failed rotate commits nothing / failed add leaves the member removed → recover via normal re-invite). Plus a per-member "Re-admit (reset key)" button in `LabRoster.tsx` (gated `MULTI_FOLDER_ENABLED && isLabHead && !isSelf && !archived`): loads the live record (`getLabRemote` + `verifyMembershipLog` before trusting), shows the current key fingerprint, resolves the member's NEW identity via `searchResearchers` (old identity greyed by fingerprint match), confirms old-vs-new, calls the orchestrator with the live session's lab key + head signing key, invalidates the roster. 4 new tests; lab suites 32 pass; tsc 0; flag-OFF byte-identical (dialog only mounts from the gated button). **No new backend route, no signed-log schema change.** Known follow-up: `searchResearchers` requires the reset member to have a findable published profile — a fingerprint-addressed "keys by fingerprint" directory read would be cleaner but is identity-lane / directory-route surface, deferred to the security-review co-design. **C2 COMPLETE.**
- **C3 — hybrid server escrow + OAuth-gated reissue (THE HEAVY ONE; gated on security review). DESIGN DOC DRAFTED 2026-06-15, awaiting Grant's sign-off: `docs/proposals/2026-06-15-c3-server-escrow-crypto-design.md`.** Key finding: the STRICT zero-knowledge tier is ALREADY BUILT (`my-backup` serves a recovery-code-wrapped blob over OAuth, server can't open it); the only new surface is the RECOVERABLE default tier, which inescapably means we hold a server-decryptable copy. The crux decision is SEK (server escrow key) custody — must live in a different trust domain than the Neon DB (KMS/HSM or asymmetric-split), else a single DB leak/subpoena yields plaintext keys. Recommend beta default = strict (zero new risk) and flip to recoverable only after custody is provisioned + reviewed. None of the C3 backend lands on shared main until sign-off. The default tier: the server can release the account's key after an OAuth re-auth, so "forgot recovery code" is not fatal. Needs a careful crypto design (a server-held release capability or a second escrow copy wrapped under a server-managed key released on verified OAuth), access controls, an audit log, and the reissue route. Today's `/api/directory/my-backup` is recovery-code-gated escrow; the hybrid adds the OAuth-gated path alongside it. **Do NOT build before the crypto design is signed off (ideally with the identity lane).**
- **C4 — tier toggle** ("Advanced Protection" opt-in strict E2E) + clear disclosure of the trade.
- **C5 — cross-device restore (STRICT PATH DONE).** On a new device with no local key but a canonical identity published elsewhere, the Phase B `// Phase C:` stop-and-point-at-recovery guard is replaced by a real restore gate (`UserLoginScreen.tsx`, merge after `b940bd8b9`): recovery code → `recoverDeviceKeyFromCloud` (unwraps the OAuth-gated my-backup blob, sets session + at-rest vault) → bind to this folder via `reuseAccountIdentityIfVerified` (defensive direct sidecar write as fallback) → `performLogin`. Same end state as same-device reuse; cancel drops to the picker, never mints a divergent key. Strict path only — no new server surface; tsc 0, identity suite 100, dark behind `NEXT_PUBLIC_MULTI_FOLDER` (trigger inside the existing flag guard), flag-OFF byte-identical. **Remaining (gated on C3):** the recoverable OAuth-only restore path (no recovery code) slots into this same gate as a second door when C3's escrow-reissue route ships.
