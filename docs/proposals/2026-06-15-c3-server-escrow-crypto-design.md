...
5. **Merge the branch** into `main` ONLY after the security pass below. First **rebase onto current main** (the branch is behind on unrelated lanes' files) and run `pnpm -C frontend run prebuild` (the wiki-coverage gate) — C3 adds only API routes + a settings subsection, no NAV page, so it should pass, but a stale branch could miss a later nav-coverage fix and that gate `fast-fails every Vercel deploy` (it is NOT caught by tsc). Keep `NEXT_PUBLIC_MULTI_FOLDER` off and `recoveryTier` defaulting to strict until staged on.
