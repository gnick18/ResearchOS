...
- ACTIVE-EXPERIMENTS BAND: DONE for the Today panel (cherry-picked `624a0f64f` -> main `3d883f287`). STILL TODO: the Home hub glance variant. NOTE (2026-06-14): the band component (`ActiveExperimentsBand`/`ExperimentCard` in `components/TodayPanel.tsx`) filters `task_type === 'experiment'`, but the demo today fixture (`lib/demo-fixtures.ts`) gives its tasks real-world type strings (`Cell culture`, `PCR`, ...), so NO demo task matches and the band cannot be device-verified in demo without also adding an experiment-typed task to the fixture. Wiring it onto the otherwise-mock Home hub is also a design call (live band over mock Today rows). Surface to Grant before building.
