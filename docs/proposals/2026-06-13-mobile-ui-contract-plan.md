...
- The 10 METHOD READERS: DONE (merged to main `5bddc1518`, 2026-06-13). All per-type readers built in `mobile/components/method/MethodReadMode.tsx` (mass_spec, qpcr, western, staining, culture, cloning, extraction, markdown, pdf, coding), each with its type accent + native data rendering, device-shot-verified against the demo seeds on emulator-5554. pcr/lc/compound were already done. KNOWN THIN SPOTS (data, not layout): the `pdf` reader's page preview is a styled placeholder (the paper-faithful PDF render is a follow-up); the `coding` reader's "Script" block shows body prose because the seed has no real code field; mass_spec/qpcr graphics are representative SVGs (seeds carry no raw spectral arrays). The flat-body reformatter (see the method phone-projection spec) is what will give real free-form markdown methods the same stepped treatment.
