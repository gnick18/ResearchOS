...
- **Phase 3: ✅ BUILT + BROWSER-VERIFIED 2026-06-14.** The typed per-row treatment on the single draw-order list (kept the list = draw order rather than literal sections, to preserve drag-reorder z-control): **tree elements** recede + a `style` pill + neutral accent + **non-removable** (no delete); **data overlays** get a blue accent + a **data-source chip** (db icon + bound column / `N cols` / Data Hub table) + delete; **highlights** get an amber accent + an annotation count badge. Plus a contextual **"Legends & keys"** section listing only the keys on the figure with per-key Shown/Hidden. Whole-repo tsc 0, 301 phylo tests. Claude-in-Chrome verify: all 5 checks PASS (typed rows, non-removable tree elements, chip tracks bound columns incl `3 cols` collapse, Legends follow layer visibility + toggle), console clean. Literal grouped *sections* (vs per-row tags) remain an optional follow-up with the draw-order tradeoff noted.
