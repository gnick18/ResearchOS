...
**Owner taxes (Grant 2026-06-16).** The LLC is a single-member pass-through, so operating profit flows to Grant's personal return: self-employment tax (~15.3%) + federal marginal + WI state. The model applies ONE editable **effective tax rate on profit** (default 35%) to positive profit only, surfacing a **take-home (after tax)** line beside the pre-tax net. Because tax applies to profit, it is **zero at or below break-even and does not move the break-even point** — it only shrinks take-home above it.
