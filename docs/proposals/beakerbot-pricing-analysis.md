...
- **The rate (PROVISIONAL placeholder).** Individuals and labs pay about **1.4x bare inference cost** (cost-recovery plus the thin Stripe/proxy buffer). Departments and institutions pay about **2x bare cost**, roughly **40 percent more per unit of AI than an individual**, and that ~0.6x gap is the sustaining surplus. Because inference is pennies, 2x a one-cent task is still about two cents, well within the modest range for AI (commonly marked up several-fold), so it is honest and not a profit center. Tunable like every number here, dial it toward 2.5x to 3x only if the free-trial subsidy or AI dev actually need it.
