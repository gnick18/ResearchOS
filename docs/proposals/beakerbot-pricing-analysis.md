...
**Caveat that gates the final price:** the token-per-task distribution is a model, not a measurement. The brief is right that we must instrument real tasks before locking numbers. The shape of the recommendation does not change, but the exact free allowance and credit-block sizing should be set from that data.
...
- The user buys a block (suggest **$10 / $25 / $50**), one Stripe charge, which amortizes the $0.30 fee (about 6% on a $10 block, about 4% on a $25 block).
- Each task debits its actual cost times a thin buffer from the balance.
- The UI shows a live dollar balance and per-task transparency (the mockup already shows "$4.10 credit left" and "last task 31k tokens"). Keep that. Dollars are honest, tokens stay visible for the curious.

### The free allowance (generous, covers casual use)
Per the brief, a generous monthly allowance should cover casual users so the AI never feels like a paywall.

- Recommend a **free monthly AI allowance of about $2 to $3 of inference per user** (roughly **150 to 300 average tasks a month**). A researcher running a handful of AI tasks a day stays free.
- The free allowance is part of the **solidarity model**, kept free for individual researchers and funded the same way the free storage tier is (institutions paying a sustaining rate, plus the LLC), NOT by marking up the paid credits. This keeps the AI markup genuinely thin.
- Labs can optionally fund a shared AI credit pool (like the storage pool, only the PI pays), so a PI can cover heavy users. Per-user free allowance still applies first.
...
Mirror the Plus/Pro storage posture: publish the model and the free allowance, but hold exact credit-block prices and the final per-task rate until real-task instrumentation lands. Label any number an estimate. Do not promise "never charge" for the AI.
...
>
> Search is free because it runs on your device. The AI is optional and metered, because each AI task costs real inference money and we pass it through near cost. Your files never leave your machine either way, BeakerBot moves answers, not your data.
...
>
> - **A free allowance every month.** Casual use stays free. You only reach for credits if you lean on the AI heavily.
...
### BILLING_FACTS.md
Add an "AI (BeakerBot)" section once numbers lock: the free-vs-metered split, the free allowance, the prepaid-credit model, the cost-recovery framing and the why, and that it is a separate meter from storage on the same LLC. Keep `BILLING_FACTS.md` canonical (per `feedback_keep_billing_facts_current`).

---

## 5. Implementation notes (for whoever wires billing)

- **The meter**: a per-user AI usage meter (tokens or dollar-cost per task), debiting a credit balance, with the monthly free allowance applied first. The proxy at `frontend/src/app/api/ai/chat/route.ts` already centralizes every AI call, so that is the natural metering point (it sees input/output token counts in the provider response).
- **The balance + free allowance**: stored per user. The lab-pool option mirrors the storage pool wiring.
- **Stripe**: prepaid credit-block purchases (one-time charges), same Stripe account as storage. No new entity.
- **Config as placeholders**: put the rates, the free allowance, and the buffer in a tunable config like `assumptions.ts`, flagged provisional, so they are config-only to change.
...
2. Grant confirms the free-allowance size and the credit-block prices.
