...
- **A one-time free trial credit of about $2 to $3 per user** (roughly 150 to 300 tasks), granted once on first use, so every user can genuinely try BeakerBot (run a few analyses, a summary, a plot) before deciding to spend. This caps our free-AI exposure at a one-time acquisition cost per user, not a recurring subsidy.
...
>
> Search is free because it runs on your device. BeakerBot is optional, with a free credit to try it, then metered near cost, because each AI task calls a hosted model that costs us real money. Your files never leave your machine either way, BeakerBot moves answers, not your data.
...
>
> - **A free trial to start.** Every account gets a one-time credit to try BeakerBot, so you can see what it does before you spend anything.
...
2. Grant confirms the one-time trial size and the credit-block prices.
