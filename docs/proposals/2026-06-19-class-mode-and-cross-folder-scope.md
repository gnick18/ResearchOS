...
REFINEMENT 3, per-folder color/badge + nicknames in the switcher (Grant, 2026-06-19). One profile can hold ALL folder kinds at once (a solo workspace, a lab they head, a lab they joined, a class they teach, a class they take), so the switcher needs a VISUAL KEY to tell them apart at a glance, plus user-chosen nicknames.
- Color/badge: each remembered-folder row gets a distinct color + a short text-pill badge derived from its `labRole` (solo / head / member / class / student). Use brand tokens, NOT new glyphs (a colored text pill avoids needing a verified-asset icon per kind). The five kinds get five accessible, visually distinct accents. This extends the existing `folderLabLabel` pill rather than replacing it. An optional per-folder manual color override can come later; the auto-by-kind color already serves the distinguishing purpose.
- Nicknames: the user can nickname ANY remembered folder. Add an additive optional `nickname?: string` on `RememberedFolderMeta` (FLAG, data-shape, additive nullable); the switcher displays the nickname when set, else the real folder name, and the underlying folder name is preserved. There is already a `renameRememberedFolder` primitive (`indexeddb-store.ts:1430`) to build on; prefer a separate `nickname` field over overwriting `name` so the original is never lost. Inline edit affordance (a pencil) per row.
- Phase placement: this is a Stage 2 switcher enhancement, sequenced AFTER the create-a-class lane (`claude/class-create-switcher`) lands so the two do not race `FolderSwitcher.tsx`. The badge/color half is NOT class-gated (it helps every multi-folder user, so it rides `MULTI_FOLDER`); the nickname half is likewise general. The color palette per kind is a visual choice to surface to Grant for sign-off.
