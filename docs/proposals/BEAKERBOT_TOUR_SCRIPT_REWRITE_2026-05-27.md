# BeakerBot Tour Script Rewrite — 2026-05-27

This is Grant's rewrite of the v4 onboarding tour script. It is the source-of-truth for every speech beat after this date.

**For sub-bots:** the prior script (with detailed per-step voice / spotlight / completion metadata) is at `docs/proposals/BEAKERBOT_TOUR_SCRIPT.md`. Use it as your reference for unchanged-step metadata. Use THIS file as the authority for:

- Speech text (every step that's still in the new ordering)
- New steps (explicitly marked `NEW:` with full voice / spotlight / completion / ExpectedRoute lines)
- Dropped steps (any step in the old script not appearing in this one)
- Dynamic-variant copy (`[DYNAMIC]` blocks)
- Wrong-click flash speech overrides
- Voice / behavior changes signaled by phrasing (e.g., "Click X" with no "Watch me" implies USER_ACTION rather than BEAKERBOT_DEMO)

**Computed diff from current TOUR_STEP_ORDER (96 steps → 102 steps):**

DROPPED (8): `home-page-intro`, `project-page-intro`, `experiment-tabs-overview`, `methods-file-vs-markdown`, `workbench-page-intro`, `workbench-create-experiment` (explicit `[DROP]` marker), `settings-page-intro`, `search-page-intro`.

NEW (7): `project-overview-rollup`, `project-overview-typing-demo` (split off the BEAKERBOT_DEMO portion of `project-overview-prose`), `notifications-intro`, `methods-lc-demo` (split off the LC editor portion of `methods-type-tour`), `hybrid-editor-scope`, `settings-intro`, `ai-helper-size-options` (split off the cursor-cycling portion of `ai-helper-size-diff`).

VOICE / BEHAVIOR CHANGES suspected (sub-bots verify against current step file):
- `methods-category-open`: was BEAKERBOT_DEMO, now USER_ACTION ("Click +New Category... I'll handle the rest"). Adds wrong-click flash.
- `workbench-create-experiment-open`: speech now ends with user-click directive; verify voice still appropriate.
- `hybrid-image-attach`: explicitly marked USER_ACTION + Spotlight `hybridEditorImageStrip` + Completion manual in the new script (likely was BEAKERBOT_DEMO before).
- `purchases-create-button-click`: speech is a user-click directive; adds wrong-click flash.

NEW DYNAMIC variants:
- `project-overview-exit`: off-home vs already-home (was static).
- `links`: Beat 1 everyone, Beat 2 lab-only (was static).

NEW WRONG-CLICK FLASH copy:
- `methods-category-open`
- `purchases-create-button-click`

The rest is mostly pure copy rewrite across the ~85 unchanged-position steps.
...
Route: `/workbench/projects/<id>`. Cursor types a placeholder hypothesis into the Overview textarea. This is the BEAKERBOT_DEMO portion that previously lived at the end of `project-overview-prose`.
...
```
Next stop: Methods. This is your library of reusable protocols. Write a technique once here, then attach it to every experiment that uses it instead of rewriting the steps each time.

To keep the library navigable, methods get sorted into categories. Let's start one based on work you actually do. What is a common technique in your lab?
```

### methods-category-open

USER_ACTION (sub-bot: verify and change voice if needed). Adds wrong-click flash.
...
Split off the LC Gradient portion of `methods-type-tour`. Cursor opens the LC Gradient editor.

- Voice: BEAKERBOT_DEMO
- Spotlight: (LC Gradient editor surface)
- Completion: manual
- ExpectedRoute: (methods catalog / LC editor)
...
Drop this step from TOUR_STEP_ORDER. The "I'll name the experiment for us." beat is retired.
...
Route: `/workbench`. Slots in immediately after `hybrid-notes-vs-results`, before `hybrid-markdown-intro`. Promoted from NARRATION to BEAKERBOT_DEMO on 2026-05-27 (hybrid editor demo fix manager, Grant hand-walk) so BeakerBot can demo the popup fullscreen affordance and actually expand the popup for the upcoming markdown demos.
...
Voice change: USER_ACTION (was BEAKERBOT_DEMO). Spotlight `hybridEditorImageStrip`. Completion manual.
...
Sub-bot: this step now opens with the section intro that used to live in the dropped `search-page-intro`.
...
USER_ACTION. Adds wrong-click flash.
