...
- **Cross-object atomic actions.** [SHIPPED] `create_experiment_chain`, `link_tasks`, `setup_experiment`, and now `setup_project` (merged `d2c843a71`, 2026-06-16) — the project-level composite that creates a project and every experiment ALREADY assigned to it in one consent, the cross-type back-reference the model cannot do by chaining separate `create_*` calls. Pure `computeProjectSetupPlan` core + 24 tests, tsc 0. (Doc previously said PARTIAL / "a fully general cross-type composite is still open".)
