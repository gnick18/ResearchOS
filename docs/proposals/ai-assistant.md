...
The agent loop runs in the browser with tools over the local folder. The model is a stateless remote inference endpoint called per turn, so only per-turn context leaves the device, never bulk data or writes. A **thin token-minting proxy** handles key custody, CORS, and per-query metering (a pure browser client cannot hold keys or use the strongest providers' contracts, and Stripe metering needs a server). This slots into the existing sharing relay infrastructure, and the bulk data still never touches it. Build against a provider-agnostic OpenAI-compatible base URL, so "serverless provider," "self-hosted GPU," and "the lab's own endpoint" are all config, not a rewrite.
