...
**Status:** BUILT + APPROVED (Grant re-approved 2026-06-15). All 3 build-order steps complete and committed on local main: (1) `smart-binding.ts` engine + 12 unit tests, (2) `SmartDataWizard.tsx` mounted in `PhyloStudio` (Add menu + auto-suggest banner), (3) BeakerBot `suggest_tree_overlays` tool (`src/lib/ai/tools/phylo-tools.ts` + `overlay-wizard.ts` + `overlay-commit.ts`, with tests). Gate green: `tsc --noEmit` 0; 328 phylo + overlay tests pass. BROWSER-VERIFIED 2026-06-15 (Grant + agent): all 8 steps PASS — banner + wizard (join chip honestly "joins 7 of 8 tips" partial), columns step with locked anchor, "Add N overlays" as separate restyleable layers, loop-back, empty state, AND the BeakerBot inline door narrating the SAME real engine number. No console errors. TWO FOLLOW-UP GAPS found (not Phase 4 regressions, pre-existing): (1) the wizard's "this collection" join scope is keyed to a NAMED project, so a tree + table both in "Unfiled" do not join (project-scope is by design per decision 2, but Unfiled-joins-Unfiled is a gap); (2) NO UI path to persist a pasted/loaded tree into a chosen project — the Phylo "Paste -> Load this tree" flow renders but does not save to the sidebar-selected collection, so a pasted tree can't be co-scoped with a project table (chip filed). Also fixed in the same verify sweep: the shared inspector rail overflowing behind the floating ask bar (commit 8255af2fc). Builds on the LOCKED Phases 0–3 findability redesign + the `layer-schema.ts` engine.
