...
> **RESOLVED 2026-06-13 (later, takeover session). The "Maximum update depth" transclusion crash was a STALE-BUILD GHOST, not a committed-code bug. The lane is now effectively DONE.** A takeover session built a stopgap (inert editor chip) + a dev harness (`?liveTransclude=1` flag forcing the live mount + a seeded `/demo` repro note), then a Claude-in-Chrome capture forced the LIVE `TransclusionEmbed` mount in a LORO-BOUND editor (`LORO_PILOT_ENABLED=true`, the handle opens for demo notes) with a non-cyclic section, on a freshly rebuilt server. It rendered the live section card cleanly: NO "Maximum update depth", no `updateEntryContent` loop, no component error. That is the EXACT CM6-widget + Loro path this doc blamed, so if it were a live bug, forcing the path reproduces it. It did not. The earlier "reproduces 100%" reading was against a dev server this doc itself documented as serving stale/concurrent code; the genuine latent setState-loop trap (the broken-image scan) was already fixed in `f3d11822b`. The stopgap + harness were fully REVERTED to baseline (commit `b017866c0`; net of 4 commits this session `b923f93ca`/`99d0d73a4`/`7f6696873`/`b017866c0`, the embeds code is byte-identical to the handoff state MINUS the crash). Live transclusion in the editor is the intended default again. tsc 0, 219 embed/cm/transclusion/fixture tests green, all on LOCAL main UNPUSHED. ONLY residual: a belt-and-suspenders real-folder transclusion sanity check (the original reports were real-folder; demo + Loro is proven clean). If clean there, Phase 7 + the whole markdown-embeds lane is fully CLOSED. Everything below this banner is the now-historical open-bug writeup, kept for context.**
