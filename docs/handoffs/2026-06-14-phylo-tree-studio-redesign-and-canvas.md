...
4. **CRUCIAL — build it as ONE shared capability with TWO front doors (GUI + BeakerBot), not two implementations.** The deterministic core (detect joinable tables, rank by coverage, enumerate the overlays each column can drive, add the chosen ones) is a reusable engine + the wizard UI. **BeakerBot must be able to spawn the SAME wizard inline in chat** when a prompt calls for it ("what data can I put on this tree?", "overlay my resistance data") — i.e. an `agent`/tool that renders the existing wizard widget in the conversation and guides the user through it, NOT a parallel LLM-only path that reinvents the join/suggest logic. The LLM only orchestrates + narrates (pick/justify a table or column, walk the user through the steps); the deterministic engine does every join-rate calc, possible-plots enumeration, and the actual add. This matches the established BeakerBot rules: **deterministic tools compute, the model only narrates / operates the app, never interprets** (`[[feedback_beakerbot_no_interpretation]]`, `[[project_beakerbot_summary_suite]]`) and the inline-widget-in-chat pattern (`[[project_beakerbot_record_set_widget]]`). So: design the engine + wizard widget so both the `/phylo` GUI and a BeakerBot tool mount the exact same thing. Coordinate the tool surface with the BeakerAI lane (they own `src/lib/ai/tools/*`).
