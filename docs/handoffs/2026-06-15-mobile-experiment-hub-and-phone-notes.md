...
1. **phone-notes P1 laptop half** — the `experiment-notes` snapshot BUILDER on the laptop (read `users/{owner}/results/task-{id}/notes.md` + `results.md`, seal, `publishSnapshot`) + a publish trigger. Mirror `frontend/src/lib/mobile-relay/method-snapshot.ts` (build + `publishMethodToAllDevices`). NOTE: the method-snapshot publish TRIGGER was not where I expected — find where the focused-method snapshot is published on experiment open and ride that path. Verifies only on a paired device.
2. **phone-notes P2** — block-list edit UI on the hub + `insert-note-block` sealed command (positioned generalization of `append-line`; `poll.ts` handler inserts at a content-anchor, Loro insert if the editor is open / `.md` splice if closed; `clientId` idempotency) + the `[!phone-note]` callout renderer in `RenderedMarkdown`.
