...
> **RESOLVED 2026-06-19 (takeover session, origin/main `b693619ea`, promoted to prod).** Both open threads are DONE + browser-verified live. (1) GATE BUG: a lab-origin bypass added to `AppContent` (`frontend/src/lib/providers.tsx`) keyed off a new `isLabPublicHost({host,enabled})` helper in `lib/social/lab-byo.ts` (same single source as the proxy, custom-domain-ready), gated on `LAB_SITES_COM_ORIGIN_ENABLED` + `mounted` (hydration-safe, byte-identical when off). (2) PERMANENCE: `LabSiteDashboard.tsx` claim step now has the permanent-address warning + a confirm checkbox showing the exact `<slug>.research-os.com`, refusing the claim until confirmed; confirmed NO user-facing rename path (POST route idempotent). VERIFIED with headless Playwright on prod: `fakeyeast-lab.research-os.com` renders the lab (no welcome overlay), `research-os.app` still shows the gate (unchanged). tsc clean, 0 lint errors, social suite 261/261 (+4). STILL OPEN, the SECONDARY Blocker 2 (the `research-os.app/<slug>` 301 does not fire). New data: the route runs per-request UNCACHED and all three flags ARE set for Production, yet the node route reads `isLabSitesComOriginEnabled()` false while the edge proxy reads it true on the same deployment (an edge-vs-node env read discrepancy, needs server-side logging to root-cause). Moot for security (the gate shows welcome on the app origin, no lab leak), affects only old-link auto-forwarding. See `[[project_lab_domains_companion_sites]]`.
