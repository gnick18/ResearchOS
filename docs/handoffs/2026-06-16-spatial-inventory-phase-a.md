...
## Phase C — the spatial 2D room map (laptop spine BUILT 2026-06-16)
The genuinely-new spatial layer (where in the ROOM, vs the logical Storage map's which-box). Grant said keep going → built the laptop spine, NOT yet browser-verified.
- **Data model + store + API** (`435cefaa7`): `LabMap = { plan, pins }` in types.ts — each `LabMapPin` marks a `StorageNode` (nodeId) or a free label at a normalized 0..1 (x,y). One record per lab, whole-lab shared, mirroring StorageNode's store/sharing/computed-union read. `labMapsApi` (list/get/create/update) + `normalizeLabMapRecord` (plan back-fill + pin clamp) + `fetchAllLabMapsIncludingShared` + `getOrCreateLabMap` (single-map idempotent). 7/7 unit tests (`frontend/src/lib/__tests__/lab-maps-api.test.ts`).
- **Room map view** (`fb19a0055`): a third inventory view (List / Storage map / Room map) — `frontend/src/components/inventory/RoomMap.tsx`. Gridded canvas + draggable pins; side rail of unpinned StorageNodes to drop; click a pin → node + subtree item count; remove pin; persists to the LabMap (debounced). The KEY architecture choice: pin CONTAINERS (freezers/benches), not items — few pins, and "where is item X" resolves item→stock→its node→that node's pin.
- **NOT browser-verified.** The inventory page reads from a connected research folder (File System Access API needs a real user gesture), so the preview harness can't reach it headlessly. Verify live: open /inventory → Room map tab → pin a freezer, drag it, reload (persists). Same "needs a real environment" gate as the mobile device writes.
- **Mobile room-map viewer + find-on-map DONE + emulator-verified** (`9b064f2fc`): the inventory snapshot now publishes the `LabMap` (plan aspect + pins) + each stock's `locationNodeId`. New read-only `mobile/app/room-map.tsx` renders the floor plan with pins; tap a pin → location + tracked-item count. THE headline "where is it": the Inventory tab's located rows are tappable ("find on map") and open the map with that item's location highlighted — it walks the stock's deep node (box→rack→freezer) up to the nearest PINNED ancestor and emphasizes it (others dimmed), so pinning a freezer once locates all its contents. Verified: tapping Puromycin (buried in a box) highlighted "-80 #2" + dimmed the fridge + showed "1 tracked item here". A "View room map" entry shows when the lab has a map. mobile tsc 0.
- **NOT yet built (Phase C/D remainder):** floorplan IMAGE import (plan.kind="image" slot exists, no upload UI); RoomPlan 3D capture → flatten-to-2D (the iOS-Pro one-person-scan layer, the last big piece). `--no-verify` used on `fb19a0055` + `9b064f2fc` because a sibling lane's dirty `LinearMap.tsx` trips the icon-guard; my files have zero inline svg.
