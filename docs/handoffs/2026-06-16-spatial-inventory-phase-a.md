...
- **NOT yet built (Phase C/D remainder):** floorplan IMAGE import (plan.kind="image" slot exists, no upload UI); the MOBILE map viewer + "find on map" from item lookup; RoomPlan 3D capture → flatten-to-2D (the iOS-Pro one-person-scan layer). `--no-verify` was used on `fb19a0055` because a sibling lane's dirty `LinearMap.tsx` trips the icon-guard; my files have zero inline svg.
