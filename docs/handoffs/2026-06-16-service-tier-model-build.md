...
1. **Publish entitlement hook — BUILT 2026-06-16 (pre-wired for their Phase 2).** `isLabPublishEntitled(labOwnerKey)` in `lib/billing/db.ts` (async, ensures schema, returns true only for an active paid LAB-audience plan, reusing the new exported pure predicate `isActiveLabPlan`). Inert until the social lane imports it. Unit-tested `__tests__/lab-entitlement.test.ts` (5 cases).
