...
- **Owner taxes** (added 2026-06-16): single-member LLC pass-through, so profit flows to Grant's personal return (SE ~15.3% + federal + WI). `taxOnProfit(net, rate)` + `DEFAULT_TAX_RATE` 0.35; `projectAtScale(...)` takes a `taxRate` param (default 0) and returns `{tax, takeHome}`. Applied to POSITIVE profit only, so it does NOT move break-even, only take-home above it. Dashboard has an "Owner tax on profit %" dial + a "Take-home (after tax)" line beside the pre-tax net.
