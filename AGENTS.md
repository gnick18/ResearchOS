...
- ✅ **`FloatingLeaveDemoButton` focus-visible ring** — closed by Batch C sweep 2026-05-14. Appended `focus-visible:ring-2 focus-visible:ring-amber-300 focus-visible:ring-offset-2` to the button className at `FloatingLeaveDemoButton.tsx:42`.
...
- **API hardening round-3 follow-ups (from the round-2 audit at `6610b7f5`)**: (a) **rate limiting via Vercel KV / Upstash** — per-IP token bucket on both proxies; currently unbounded, can be DoS'd or abused as a low-volume open proxy. In-memory store would die on cold starts. (b) **IP-pinned dispatch to close the DNS-rebinding TOCTOU** — would require a custom undici Dispatcher; only worth doing if SSRF posture must be airtight for the public deploy. (c) **OAuth callback routes audit (round 3)** — `/api/auth/{google,microsoft}/{login,callback,refresh}` haven't had a security pass; cookie attributes (HttpOnly/SameSite=Lax/Max-Age=600) look correct but the `postMessage` payload to `window.opener` deserves an explicit `origin` check. **Note**: (c) is now obsolete — the Google/Microsoft OAuth surface was removed entirely 2026-05-14 (see `/api/auth/{google,microsoft}/*` deletion in handoff snapshot). ✅ (d) **Force HTTPS for ICS feeds** — closed by Batch C sweep 2026-05-14. `allowedSchemes` in `app/api/calendar-feed/route.ts:67` is now `["https:"]`; a raw `http://` URL gets a clear 400 with "Calendar feeds must use HTTPS for security." `webcal://` / `webcals://` still rewrite to `https://` upstream of the guard. ✅ (e) **Tighten Telegram path length** — closed by Batch C sweep 2026-05-14. Cap in `app/api/telegram-file/route.ts:53` lowered from 512 → 80 chars.
