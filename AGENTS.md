...
**Export-revamp manager (parallel session, not this chat)** — final report delivered 2026-05-13 evening, export-arc closed. Seven sub-bots spawned (A–G), all integrated; one chip queued (H — PCR cross-instance round-trip) awaiting Grant's decision to run or skip. See the "Export revamp — full arc shipped" entry in the Recently-landed list below for the integration breakdown.
...
- **Export revamp — full arc shipped** (commits `f77155dd` → `b3782349`, ~3,700 LOC under `lib/export/` + `lib/import/` + dialogs): old 622-line html2canvas-rasterized `export-utils.ts` deleted; new three-format pipeline (PDF via `@react-pdf/renderer`, HTML via `marked`, Raw bundle for cross-instance) under `frontend/src/lib/export/{types,slug,markdown,extract,orchestrate,raw,html,pdf}.ts`. Receiver-side import flow under `frontend/src/lib/import/`. New UI: `<ExportFormatDialog>` three-card picker, `<ImportExperimentDialog>` resolution preview, multi-select on `/search` and `LabSearchPanel`. Method-id matching replaced name-substring lookups (`methodId?: number` on `ExperimentAttachment`); PCR rendering pre-fetches `PCRProtocol` (`pcrProtocol?: PCRProtocol | null` on `MethodPayload`); `pcrApi.get` gained optional owner for shared-task private-protocol routing. Seven sub-bots A–G integrated; chip H (PCR cross-instance round-trip) queued.
