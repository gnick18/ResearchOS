...
- **`SidebarTree.tsx` is dead code.** Spotted during the gantt-prop-migration follow-up (`b086fb4c`): only reference to `SidebarTree` outside the file itself is a stale comment in `app/gantt/page.tsx:166`; no imports. The component file is ~250 lines including its own prop types. Safe to delete in a cleanup pass — confirm with `grep -rln "SidebarTree" frontend/src` first.
...
- **method_attachments orphan rows on disk.** Audit found yeast demo task 2 with `method_ids: []` but `method_attachments: [{ method_id: 1, ... }]` — symptom of the method-detach handler not pruning the attachment row when the method is removed. Sub-bot `claude/elegant-curie-92d6da` spawned a B2 follow-up (chip queued 2026-05-14 evening, prompt at the manager session's tail) to: (a) filter at export time in `extract.ts`, (b) audit the upstream detach handler + enforce the invariant `∀ a ∈ method_attachments: a.method_id ∈ method_ids` at the `tasksApi.update` boundary, (c) lazy-normalize at read via `normalizeTaskRecord`. Confirm the demo seed at `scripts/generate-demo-data.mjs` isn't producing this state.
