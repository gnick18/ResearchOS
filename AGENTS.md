...
- **`pcrApi.update` may have the same private-first issue** (flagged by pcrApi.get owner-threading bot, 2026-05-14). Writes from a receiver editing a shared PCR protocol could land in the receiver's private store instead of the owner's. Out-of-scope for the get-fix chip but worth a follow-up. Mirror the same `owner?: string` argument the get fix landed.
- **`pcrApi.get` priority ordering bug** ✅ closed by `1d122fc0` (owner threading added, no private-then-public fallback when owner provided). Below entry retained for context.
- **(closed)** **`pcrApi.get` priority ordering bug** (flagged by PCR-blank debugger 2026-05-14). `pcrApi.get(id)` reads private protocols before public — so when alex opens a task attached to **public** method 2 (DemoCheck PCR), alex's PRIVATE `pcr_protocols/1` (qPCR fakeGFP) gets returned by the read instead, and the wrong protocol renders. Symptom: attached method label says one thing but the gradient/recipe is a different protocol's data. Needs an explicit owner/scope argument in the read or a priority change. Affects any user with a private protocol whose id happens to match a public protocol's id (a real collision in the demo since alex has private pcr_protocols/1 AND public has pcr_protocols/1).
