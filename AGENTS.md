...
- **Deterministic export bytes.** Re-export of the same task currently differs only in zip-entry mtimes (verified by md5 on inner HTML — identical; outer zip — different). Passing `payload.meta.exportedAt` as `date` to JSZip's `generateAsync` would make re-exports hash identically. Useful for caching / dedup tooling. ~3 lines in `frontend/src/lib/export/{raw,html,orchestrate}.ts`.
- **HTML + PDF manifest marker.** Currently only the Raw bundle has `_export-manifest.json`. Adding a tiny manifest inside HTML and PDF outputs too (just `{ format: "html" | "pdf", version: 1, exported_at, source_owner, task_id }`) would let downstream tools detect provenance without sniffing file structure. ~10 lines per format generator. Useful if anyone ever automates over exports.
...
- **PDF artifact audit gap.** All four supplied zips were Raw or HTML — no PDF was provided so PDF-only checks (Inter font via jsdelivr fetched correctly, selectable text via `pdftotext`, bookmarks via `pdfinfo`, PCR table appendix layout) are unaudited. Re-export of any task as PDF should be inspected with `pdftotext file.pdf -` (text non-empty), `pdffonts file.pdf` (Inter listed), `pdfinfo file.pdf` (TOC entry count). If Inter isn't listed, the jsdelivr fetch is failing silently and the PDF is using the Helvetica fallback.
- **method_attachments orphan rows on disk.** ✅ Closed by B2 sub-bot (`claude/clever-darwin-69f05b`, integrated on manager branch). Investigation: no live runtime detach path leaks orphans — `tasksApi.removeMethod` already prunes both arrays. The state on disk came from the demo seed `scripts/generate-demo-data.mjs:735-736` hard-coding `method_ids: []`. Generator fixed (derive `method_ids` from `method_attachments`); on-disk demo regenerated for the 7 affected tasks (alex 2/5/10/11, morgan 1/2/3); invariant enforced at `tasksApi.update`; lazy-normalize on read in `normalizeTaskRecord`; belt-and-braces filter in `extract.ts`. Awaiting Grant's auth to merge to main.
- **Latent shared-task method-detach owner routing** (flagged by B2 sub-bot, out of scope for B2 itself). `frontend/src/components/MethodTabs.tsx:8` imports `tasksApi` from `@/lib/local-api` directly rather than the `ownerScopedTasksApi` wrapper that `TaskDetailPopup` uses for other mutations. After B2 the orphan invariant is enforced regardless of owner, so this won't create orphans — but it does mean a receiver editing a shared task may route `addMethod`/`removeMethod` to their own user's directory instead of the owner's. Worth a 5-minute confirmation: open a shared task as the receiver, attach a method, check whether the write landed under `users/{receiver}/tasks/...` (bug) or `users/{owner}/tasks/...` (correct). Fix is one-liner if confirmed (swap the import to `ownerScopedTasksApi`).
