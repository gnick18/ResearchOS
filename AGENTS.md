...
- **File attachments redesign** — `claude/musing-herschel-85c2f1`, commit `210c4142`. FileStrip + Images/Files tabs in `LiveMarkdownEditor`. **Reported back; awaiting live-UI verify before merge.** Bot did not run a browser test (sandbox limitation); user needs to walk the golden path (drop a PDF into the editor → confirm FileStrip lists it → drag into body → confirm link inserts and opens). Off-limits while unmerged: the attachment ribbon, the markdown editor's drag handlers.
