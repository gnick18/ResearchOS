...
- **Wiki page: `/wiki/getting-started/labarchives-export`** (HR EOD note + ELN domain). New how-to page covering how a user generates the offline ZIP from LabArchives in the first place, since the ELN import feature consumes that format. Hand off to wiki manager after Grant signs off the wizard verification.
