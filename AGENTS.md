...
- **File attachments stack** — `claude/vigilant-bardeen-54573f`, commit `e89f347b`. Stacks three commits on top of `claude/musing-herschel-85c2f1`: (1) FileStrip + Images/Files tab in `LiveMarkdownEditor`, (2) native OS file drops wired across TaskDetailPopup + methods/page, (3) auto-dismissing toast for surfaces that don't support file drops yet (NoteDetailPopup, MethodTabs variation notes). **Reported back; awaiting live-UI verify before merge.** Methods use a root-level `{root}/methods/{slug}/Files/` convention mirroring the existing `Images/` shape. Off-limits while unmerged: attachment ribbon, markdown editor drag handlers, `filesApi.listDirectory`, `fileEvents` bus.
- **Wiki feature-page rewrites** — `claude/naughty-pascal-f22111`, commit `351a7957`. Concept-first rewrites of 12 feature/integration pages, plus enriched `wiki-capture-fixture.ts` to give screenshots structure. **Reported back; awaiting user eyeball at `/wiki/features/*` + screenshot bot to land 6 new PNGs.** Off-limits: `frontend/src/app/wiki/features/`, `frontend/src/app/wiki/integrations/`, `frontend/src/lib/file-system/wiki-capture-fixture.ts`.
- **Wiki screenshot capture (6 new PNGs)** — branch TBD, spawned by the wiki writer bot. Will add 6 routes to `scripts/capture-wiki-screenshots.mjs` and capture `gantt-task-popup`, `gantt-zoom-controls`, `home-project-popup`, `experiments-editor`, `purchases-funding-modal`, `lab-mode-activity`. Off-limits: `scripts/capture-wiki-screenshots.mjs`, `scripts/WIKI_SCREENSHOTS.md`.
- **ResultsEditor + `/results` consolidation** — branch TBD, chip queued. Audits whether to delete the legacy `ResultsEditor` (Strategy A), unify storage on `Files/` while keeping the component (B), or keep `Attachments/` as a separate concept (C). Off-limits when active: `frontend/src/components/ResultsEditor.tsx`, `frontend/src/app/results/page.tsx`, the `Attachments/` folder convention.
