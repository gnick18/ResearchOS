...
- **Wiki content / voice (managed by parallel manager session)** — not this session. Recent landings 2026-05-13: concept-H2 voice fixes (architect-speak removal), `/wiki/features/markdown-editor` page, Results wiki rewrite against the new `/results` UI, PCR wiki + Methods nesting, fixture infrastructure (binary PNGs, Lab user filter), demo-lab fixture overhaul (yeast synthetic-biology — replaces the fungal-themed wiki screenshots). Off-limits to this session: `frontend/src/app/wiki/`, `frontend/src/components/wiki/`, `scripts/capture-wiki-screenshots.mjs`, `frontend/src/lib/file-system/wiki-capture-fixture.ts`. If you need to touch wiki, coordinate first.

(Calendar / Demo Lab / file-attachments / per-tab isolation / stamp redesign / note attachments / ResultsEditor consolidation all landed on `main` 2026-05-13 — see `git log` for hashes.)
