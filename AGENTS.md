...
- **Demo Lab overhaul** — chip queued (2026-05-13, not yet spawned). Replaces `wiki-capture-fixture.ts` with a yeast synthetic-biology lab clearly branded as "DEMO" (project names `DEMO:`, watermarked PNGs, `_demo_marker.json`, in-app warning banner). Outputs an on-disk `frontend/public/demo-data/` tree and a `public/demo-lab.zip` for visitors to download, plus a "Try the Demo Lab" button on the user-picker. **Reason for this work**: the previous fixture was themed too close to Grant's real research (BGC, ICS, *Amanita*) and rendered screenshots that looked like leaked data even though they weren't. Off-limits while in flight: `frontend/public/demo-data/`, `wiki-capture-fixture.ts`, `ResearchFolderSetupNew.tsx`, any new `scripts/build-demo-zip.mjs` or `scripts/generate-demo-images.mjs`.
- **Wiki screenshot recapture** — queued behind Demo Lab. The previous screenshot bot's PNGs were branded with Grant's research themes and were wiped (branch `claude/hardcore-albattani-9ee035` deleted 2026-05-13). The next screenshot bot will run against the Demo Lab fixture after it lands.
- **Note popup attachment GC** — `claude/<new>` chip queued 2026-05-13. Wires `gcUnreferencedAttachments` into `NoteDetailPopup` so orphaned files / images get pruned on debounced-save flush and on popup close. Recommend extracting the GC helper to `lib/attachments/gc.ts` and importing from both `NoteDetailPopup` and `TaskDetailPopup`. Off-limits while in flight: `NoteDetailPopup.tsx`, the new `lib/attachments/gc.ts`, and the matching imports in `TaskDetailPopup.tsx`.
...
- **NoteDetailPopup file/image attachments** (`fec7b2ce`): note popups now write to `users/<you>/notes/<noteId>/Files/` and `Images/`, matching the TaskDetailPopup pattern. GC not yet wired (see in-flight chip).
